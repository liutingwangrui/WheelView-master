...
public class Common {
    ...
    public static final String dateToStr(Date date){
        ...
        String time=split[1]+"月"+split[2]+"日";
        return  time+getWeek(sdf.format(date));
    }
    ...
    public static ArrayList buildMinuteListStart(TimeRange timeRange ) {
        ...
        Calendar calendarStart = Calendar.getInstance();
        calendarStart.setTime(dateStart);
        calendarStart.add(Calendar.MINUTE, 15);//分钟需要取整，5.55则从6:00开始

        int minStart = (calendarStart.get(Calendar.MINUTE) / 15) * 15;//取整
        ...
        if (isInSameHour(calendarStart, calendarEnd)) {
            minEnd = (calendarEnd.get(Calendar.MINUTE) / 15) * 15;
        }else{
            minEnd = 50;
        }
        ArrayList minList = new ArrayList<>();

        for (int i = minStart; i <= minEnd; i += 15) {
            if (i==0){
                minList.add("00" + "分");
            }else{
                minList.add(i + "分");
    ...
    public static ArrayList buildMinuteListEnd(TimeRange timeRange) {
        ...
        int minEnd = (calendar.get(Calendar.MINUTE) / 15) * 15;
        ArrayList minList = new ArrayList<>();

        for (int i = 0; i <= minEnd; i += 15) {
            if (i==0){
                minList.add("00" + "分");
            }else{
                minList.add(i + "分");
